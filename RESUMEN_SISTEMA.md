...
**⚠️ Presupuesto de funciones serverless: 12/12, plan Hobby de Vercel (0 libres).** Cada archivo `.js` no prefijado con `_` bajo `api/` cuenta como una función; hoy son exactamente 12 (`api/admin/request-password-reset.js`, `api/admin/users.js`, `api/emails/send-invoice-ride.js`, `api/emails/webhook.js`, `api/sri/reconcile-invoice.js`, `api/sri/retry-pending.js`, `api/sri/status.js`, `api/sri/submit-credit-note.js`, `api/sri/submit-invoice.js`, `api/sri/submit-retention.js`, `api/sri/upload-certificate.js`, `api/sri/verify-supplier-document.js`). Ya se llegó a este límite dos veces sin agregar un archivo nuevo: `api/admin/users.js` es un *dispatcher* que reemplazó 5 endpoints separados (`action` en el body decide qué rama corre), y `api/sri/retry-pending.js` sirve dos crons con lógicas distintas (reintentos SRI cada 15 min y evaluación de trial diaria) diferenciados por `?job=` en la query string del cron. **Antes de cualquier proyecto grande nuevo** (Cuentas por Cobrar, Contabilidad General, Nómina — cada uno probablemente necesita varios endpoints propios), hay que decidir entre: (a) seguir consolidando en dispatchers por `action`/query param, cada vez más difícil de mantener legible, o (b) subir del plan Hobby de Vercel a Pro (levanta el límite de 12) antes de que un endpoint nuevo quede bloqueado por este techo.
