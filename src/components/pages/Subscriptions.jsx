...
export default function Subscriptions() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 text-zinc-300">
      <h1 className="text-2xl font-bold text-[var(--text-primary)]">Suscripciones y planes</h1>
          ...
          return (
              ...
              {isEditing ? (
                <div className="space-y-4 mb-6">
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)] mb-1">Nombre del plan</label>
                    <input type="text" value={editData.name} onChange={e => setEditData({...editData, name: e.target.value})} className="w-full bg-[var(--surface-0)] border border-[var(--border-subtle)] text-[var(--text-primary)] rounded-xl px-3 py-2 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[var(--brand)]" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)] mb-1">Precio mensual (USD)</label>
                    <input type="number" value={editData.price} onChange={e => setEditData({...editData, price: e.target.value})} className="w-full bg-[var(--surface-0)] border border-[var(--border-subtle)] text-[var(--text-primary)] rounded-xl px-3 py-2 font-bold text-lg focus:outline-none focus:ring-2 focus:ring-[var(--brand)]" min={0} />
                  </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="flex items-baseline mt-2">
                    <span className="text-4xl font-black tracking-tight text-[var(--text-primary)]">{formatUSD(plan.price)}</span>
                    <span className="text-[var(--text-muted)] font-medium ml-1">/mes</span>
                  </div>
                </div>
              )}

              <ul className="space-y-3 text-sm text-zinc-400 border-t border-[var(--border-subtle)]/50 pt-5 font-medium">
      ...
      <div className="bg-[var(--surface-1)] rounded-3xl border border-[var(--border-subtle)] p-6">
        <h2 className="text-lg font-bold text-[var(--text-primary)] mb-4">Ingresos Mensuales Recurrentes estimados: <span className="text-[var(--brand)]">{formatUSD(mrr)}</span></h2>
        <p className="text-sm font-medium text-zinc-400 mb-6">
          ...
          {plans.map((p, i) => {
            ...
            return <span key={p.id} className="text-zinc-300"> {count} × {p.name} ({formatUSD(p.price)}){i < plans.length - 1 ? <span className="text-[var(--text-faint)]"> +</span> : ''}</span>;
          })}
          <span className="text-[var(--text-muted)]"> = </span><span className="text-[var(--brand)]">{formatUSD(mrr)} MRR</span>
        </p>
        
        <div className="w-full h-3 rounded-full bg-[var(--surface-0)] overflow-hidden flex border border-[var(--border-subtle)]/50">
          {plans.map(p => {
            const count = activeCompanies.filter(c => c.planId === p.id).length;
            if (count === 0) return null;
      ...
      <div className="bg-[var(--surface-1)] rounded-3xl border border-[var(--border-subtle)] overflow-hidden">
        <div className="px-6 py-5 border-b border-[var(--border-subtle)] bg-[var(--surface-0)]/30">
          <h2 className="text-lg font-bold text-[var(--text-primary)]">Estado de suscripciones</h2>
        </div>
        <Table 
