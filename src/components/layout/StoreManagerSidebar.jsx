...
import { LayoutDashboard, Package, TrendingUp, FileText, Users, Settings, LogOut, Building2 } from 'lucide-react';
...
export default function StoreManagerSidebar() {
  ...
  return (
    <div className={`${sidebarCollapsed ? 'w-20' : 'w-72'} bg-gradient-to-b from-zinc-900 to-zinc-950 border-r border-zinc-800 flex flex-col transition-all duration-300`}>
      {/* Header */}
      <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
        <div className={`flex items-center gap-3 ${sidebarCollapsed ? 'hidden' : ''}`}>
          ...
          <div>
            <div className="text-sm font-bold text-white">Gerente Tienda</div>
            <div className="text-xs text-blue-400">{currentUser?.name}</div>
          </div>
        </div>
        <button
